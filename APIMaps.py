...
"""Function: Does Request operations to return specific dictionaries from the Google Places API.
...
def Results(Location,Run):
	...
	if Run == "Maps":
		...
		Details_Req = requests.get(Details_Api,Details_Input) # Requests the Details dictionary.
...
"""Function: Takes the user's "location" input and sets "Address" to be the formal format of the location requested from the API dictionary.
...
def Format_Adr(Location):
	...
	return Address # Returns to "Google" function in "Location.py".
