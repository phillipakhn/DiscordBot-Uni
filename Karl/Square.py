...
async def Square(message):
	if message.content.startswith('!Square'):#This is what the user has to enter to so that this can run. 
		...
		print(Squaremsg)
		SquaredNum = int(Squaremsg) * int(Squaremsg)#This just multiples the number by itself to get the squared value of it.
		SquareNumAns = "The square number for the number" + " " + str(Squaremsg) + " " + "is" + " " + str(SquaredNum)#This message will be outputted to the user once they have ran the code
