...
'''Function: Extracts the location from the User's "message" input and creates the msg to be sent to the Discord channel.
		...
		while True:
			try:
				...
				if Run == "Maps":
					...
					msg = "The current time in " + maps.Format_Adr(Location) + " is "+ str(Content[0]) + " " + str(Content[2]) + " taken from " + str(Content[1]) # Content[0] is the time, Content[1] is the time zone and Content[2] is the city it's referencing.
					return msg # Returns to Discord.py
				else:
					...
					Celsius = round((Content['main']['temp'])- 273.15,1) # Converting Kelvin to Celsius
...
"""Function: Creates a "Condition" string which depends on what value Temp is and if Rain is True or Flase.
...
def Advice(Celsius,Rain):
	if -20 < Celsius < 0:
		...
		Condition = "It's beyond FREEZING! Only go outside if it's necessary and make sure to wear very insulated clothes."
	elif 0 < Celsius < 16:
		Condition = "It's CHILLY out there! Make to sure put a jumper on!"
		if Rain == True:
		...
		Condition = "It's nice and warm! Have fun but make sure to put sun cream on if you burn easily."
		if Rain == True:
			Condition = "It's nice and warm! However, it's raining so don't forget your coat!"
	elif 30 < Celsius < 51:
		Condition = "It's very HOT out there! Perfect holiday weather! Make sure to use sun cream and drink plenty of water!"
		if Rain == True:
